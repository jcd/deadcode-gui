...
import deadcode.gui.widget : Widget, WidgetID, NullWidgetID;
...
class Window : deadcode.gui.widget.Widget
{
	...
	override void draw()
	{
		version(Profiler) import util.profile;
		{
			version(Profiler) auto frameZonex = Zone(profiler, "Draw1");
			_renderTarget.render(false);
		}
		{
			Widget.drawCalls = 0;
			Widget.drawFeatureCalls = 0;
			version(Profiler) auto frameZonex = Zone(profiler, "Draw2");
			super.draw();
			version(Profiler) frameZonex.variableEvent!"drawCount"(Widget.drawCalls);
			version(Profiler) frameZonex.variableEvent!"drawFeatureCount"(Widget.drawFeatureCalls);
		}
		{
			version(Profiler) auto frameZonex = Zone(profiler, "SwapBuffers");
