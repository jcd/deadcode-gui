...
public:
	...
	@property
	{
		...
		void fileManager(FileManager fm)
		{
			_fileManager = fm;
		}
	}
...
*/
	class DefaultLoader : IResourceLoader!T
	{
		bool load(T r, URI uri)
		{
            ...
            enforceEx!ResourceException(_fileManager, format("IOManager not set on %s", this.stringof));
			...
			foreach (s; _serializers)
			{
				...
				{
					auto fileAccess = _fileManager.open(uri, IOMode.read);
					if (fileAccess is null)
						continue;
					s.deserialize(r, fileAccess);
					fileAccess.close();
					return true;
				}
			}
		...
		bool save(T r, URI uri)
		{
			...
			foreach (s; _serializers)
			{
				...
				{
					auto fileAccess = _fileManager.open(uri, IOMode.write);
					if (fileAccess is null)
						continue;
					s.serialize(r, fileAccess);
					fileAccess.close();
					return true;
				}
			}
	...
	*/
    ...
    {
        int result = 0;
        foreach (v; _resourcesByHandle)
        {
            result = dg(v.resource);
            if (result)
                break;
        }
        return result;
...
protected:
	...
	FileManager _fileManager; // Need a file manager because resources can be loaded/unloaded at will
}
...
class ResourceSerializer(T)
{
	...
	*/
	...
	void serialize(T res, FileAccess fileAccess)
	{
		auto output = appender!string();
		serialize(res, output);
		fileAccess.writeText(output.data);
	}
	...
	void deserialize(T res, FileAccess fileAccess)
	{
		auto str = fileAccess.readText();
		deserialize(res, str);
	}
...
version(unittest)
{
	...
	class DummySerializer : ResourceSerializer!Dummy
	{
		...
		override void deserialize(Dummy res, FileAccess fileAccess)
		{
