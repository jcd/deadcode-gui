...
{
	return deadcode.gui.keycode.isPressed(m, KeyMod.shift);
	//return (m & KMOD_SHIFT) != 0;
}

bool isCtrlDown(KeyMod m) pure nothrow @safe
{
	return deadcode.gui.keycode.isPressed(m, KeyMod.ctrl);
	//return (m & KMOD_CTRL) != 0;
}

bool isAltDown(KeyMod m) pure nothrow @safe @nogc
{
	return deadcode.gui.keycode.isPressed(m, KeyMod.alt);
	//return (m & KMOD_ALT) != 0;
}
...
class MouseMoveEvent : MouseEvent
{
	...
	@NoSerialize Widget atWidget;
	@NoSerialize Widget lastWidget;
}
...
class MouseOverEvent : MouseEvent
{
	...
	@NoSerialize Widget overWidget;
	@NoSerialize Widget outWidget;
}
