...
//	Variant data;
	...
	{
		immutable string[string] typeRenames = [
			"application.Application.SessionData" : "applicationsession.SessionData"
		];

		// Object helperObj = Object.factory("gui.resources.generic.GenericResource.Helper!(" ~ typeName ~ ").Helper");
		TypeInfo_Class* ci = typeName in typeNameToHelperMap;
		if (ci is null)
		{
			if (auto n = typeName in typeRenames)
			{
				ci = *n in typeNameToHelperMap;
			}		
		}
...
{
	static GenericResourceManager create(FileManager fileManager)
	{
