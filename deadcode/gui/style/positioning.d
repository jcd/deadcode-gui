module deadcode.gui.style.positioning;
...
private const(Vec2f) calcBaseSize(Widget w, Widget positionReference, CSSPosition cssPos)
{
	Vec2f res = w.parent.size;
	final switch (cssPos)
	{
		case CSSPosition.fixed:
			res = positionReference.window.size;
			break;
		case CSSPosition.absolute:
			return positionReference.size;
		case CSSPosition.relative:
			break;
		case CSSPosition.static_:
			break;
...
public FlexSize calcSize(Widget w, Widget positionReference)
{
	...
	return calcSizeFromBaseSize(w, positionReference);
}
...
public FlexSize calcSizeFromBaseSize(Widget w, Widget positionReference)
{
	Style st = w.style;
	CSSPositionMix posMix = st.position;
	Vec2f baseA = calcBaseSize(w, positionReference, posMix.cssPositionA);
	Vec2f baseB = calcBaseSize(w, positionReference, posMix.cssPositionB);
    Vec2f intrinsicSize = w.intrinsicSize;
    ...
    float baseSizeX = intrinsicSize.x.isNaN ? baseA.x : intrinsicSize.x;
	float mixHorzA = baseSizeX;
	if (autoSizeA.x.isNaN)
		calcWidth!(0,"width")(mixHorzA, st);
	else
		mixHorzA = autoSizeA.x;

    float minW = baseSizeX;
    calcWidth!(0,"minWidth")(minW, st);

    float maxW = baseSizeX;
    ...
    float baseSizeY = intrinsicSize.y.isNaN ? baseA.y : intrinsicSize.y;
	float mixVertA = baseSizeY;
	if (autoSizeA.y.isNaN)
		calcHeight!(0,"height")(mixVertA, st);
	else
		mixVertA = autoSizeA.y;

    float minHeight = baseSizeY;
    calcHeight!(0, "minHeight")(minHeight, st);

    float maxHeight = baseSizeY;
