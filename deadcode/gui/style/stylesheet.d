module deadcode.gui.style.stylesheet;
...
import deadcode.gui.style.parser;
...
/** Create a stylesheet that matches all and paints pink background */
StyleSheet createFallbackStyleSheet(Font font, Material mat)
{
	import deadcode.graphics.color;
    import deadcode.math.rect;
	auto sheet = new StyleSheet;
    auto rule = new Rule;
    rule.selectors ~= new StylableSelector("*", null);
    sheet.rules ~= rule;

    auto s = new Style(sheet);
    rule.style = s;
    s.color = Color.white;
    s.backgroundColor = Color.pink;
    s.background = mat;
    s.font = font;
    s.padding = RectfOffset(0,0,0,0);
