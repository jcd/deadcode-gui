module deadcode.gui.style.parser;
...
import deadcode.gui.resources.material : Material, MaterialManager;
import deadcode.gui.resources.font : Font, FontManager, TTFFontLoader;
...
class StyleSheetParser
{
	...
	*/
		...
		switch (uriStr.extension)
		{
			...
			case ".ttf":
				...
				if (style._font is null)
					style.font = fontManager.declare(theURI, TTFFontLoader.singleton);
                else
                    style._font.manager.set(style._font.handle, theURI);
			   ...
			   curToken.type == TokenType.number)
		{
			switch (curToken.type)
			{
				...
				case TokenType.number:
					...
					// Create a custom font and use a dummy loader for that
					if (style._font is null)
						style.font = fontManager.declare(TTFFontLoader.singleton);
	...
	void parseDisplayProperty(Style style)
	{
		requireNextToken(TokenType.identifier);
        ...
        switch (curToken.value)
        {
            ...
            case "initial":
				...
				//    break;
            default:
                addError("Invalid display value ", line);
                break;
        }
	}
