module deadcode.gui.style.types;
...
struct CSSVisibility 
{
    @NonBindable()
    enum invalid = CSSVisibility(0);
    
    @NonBindable()
    enum visible = CSSVisibility(1);
    
    @NonBindable()
    enum hidden = CSSVisibility(2);

    byte value;
    alias value this;

    CSSVisibility interpolate(CSSVisibility endValue, float delta)
    {
        if (endValue == this)
            return endValue;

        // To support fading we treat transition from visible to hidden and vice verse different.
        if (this == visible && delta >= 1)
            return hidden;
...
struct CSSDisplay
{
    @NonBindable() enum invalid = CSSDisplay(0);
    @NonBindable() enum box = CSSDisplay(1);
    @NonBindable() enum none = CSSDisplay(2);

    byte value;
    alias value this;

    CSSDisplay interpolate(CSSDisplay endValue, float delta) const
    {
        if (endValue == this)
            return endValue;
...
struct CSSScale
{
    ...
    float calculatePixels(float totalSize)
    {
        if (unit == CSSUnit.pct)
            return value * totalSize;
        else
        {
            assert(unit == CSSUnit.pixels);
            return value;
        }
    }
...
struct CSSScaleMix
{
    ...
    CSSScaleMix interpolate(CSSScaleMix endValue, float delta)
    {
...
struct CSSPositionMix
{
    ...
    CSSPositionMix interpolate(CSSPositionMix endValue, float delta)
    {
