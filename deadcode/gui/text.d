...
import deadcode.gui.textlayout;
...
class TextModel
{
	...
	private SubModel GetTextSubModel(Material mat, deadcode.graphics.texture.Texture fontMap)
	{
...
class RegionSetDecoration : Stylable
{
	...
	void update(int textOffset, Vec2f containingBoxSize)
	{
		...
		foreach (i, ref line; lines)
		{
			...
			if (height.isValid)
			{
				assert(height.unit == CSSUnit.pixels || height.unit == CSSUnit.pct); // TODO: support other heights as well
				if (height.unit == CSSUnit.pct)
                    curRect.h *= height.value;
                else
                    curRect.h = height.value;
			}
